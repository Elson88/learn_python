...
# def save_to_file(lista):
#     with open('lista_empregos.csv', 'w', newline='', encoding='utf-8-sig') as csvfile:
#             # Inicializa o escritor CSV
#             writer = csv.writer(csvfile, fiedl delimiter=';', lineterminator='\n')

#             # Escreve os headers em uma linha
#             writer.writerow(lista.keys())

#             # Escreve os valores correspondentes em colunas separadas
#             writer.writerow(lista.values())
...
def save_to_file(lista):
    with open('lista_empregos.csv', 'w', newline='', encoding='utf-8-sig') as csvfile:
            # Inicializa o escritor CSV
            writer = csv.DictWriter(csvfile, fieldnames=lista[0].keys(), delimiter=';', lineterminator='\n')
...
url_begin = 'https://www.net-empregos.com/pesquisa-empregos.asp?chaves=&cidade=&categoria=0&zona=0&tipo=0'
s = HTMLSession()

def getdata(url_begin):
    r = s.get(url_begin)
    soup_html = BeautifulSoup(r.text, 'html.parser')
    return soup_html


def get_next_page(soup_p):
    page = soup_p.find('ul', {'class': 'pagination'})
    #print('page:' , page)
    if page:
        last_li = page.find('a', {'class': 'page-link oferta-link d-none d-lg-block'})
        print('last_li', last_li)
        if last_li:
            url_next = 'https://www.net-empregos.com' + last_li['href']
            return url_next
    return None

#print("html", getdata(url_begin))


#soup_page = getdata(url_begin)
# print('soup page', soup_page)

#print('get next page', get_next_page(soup_page))

save_to_file(find_jobs())